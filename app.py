...
def predict_emotion():
    ...
    # Get the input text of the POST request
    input_text = request.json.get("text")  
    
    if not input_text:
        # Response to send if input_text is not defined
        response = {
                    "status": "error",
                    "message": "Please, insert a comment to predict emotion!"
                  }
        return jsonify(response)
    else:  
        predicted_emotion, predicted_emotion_img_url = predict(input_text)
        
        # Response to send if input_text is not undefined
        response = {
                    "status": "success",
                    "data": {
...
def save_entry():
    ...
    # Get data, predict emotion and the text entered by the user to save an entry
...
def bot():
    ...
    # Call the method to get the bot's response
